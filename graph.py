class Graph(object):
    def __init__(self, entities, triggers, relations, roles, nested_roles, vocabs, mentions=None):
        """
        ...
        self.roles = roles
        self.nested_roles = nested_roles
        self.vocabs = vocabs
        ...
        self.nested_role_num = len(nested_roles)
    ...
    def __eq__(self, other):
        if isinstance(other, Graph):
            equal = (self.entities == other.entities and
                     ...
                     self.nested_roles == other.nested_roles and
    ...
    def to_dict(self):
        ...
        flat_role_itos = {i: s for s, i in self.vocabs['flat_role_type'].items()}
        nested_role_itos = {i: s for s, i in self.vocabs['nested_role_type'].items()}
        mention_itos = {i: s for s, i in self.vocabs['mention_type'].items()}

        if len(self.entity_scores) < len(self.entities):
            self.entity_scores.extend([1 for _ in range(len(self.entities) - len(self.entity_scores))])
        if len(self.mentions) < len(self.entities):
            self.mentions.extend(['entity' for _ in range(len(self.entities) - len(self.mentions))])
        if len(self.trigger_scores) < len(self.triggers):
            self.trigger_scores.extend([1 for _ in range(len(self.triggers) - len(self.trigger_scores))])
        if len(self.role_scores) < len(self.roles):
            self.role_scores.extend([1 for _ in range(len(self.roles) - len(self.role_scores))])
        if len(self.nested_role_scores) < len(self.nested_roles):
            self.nested_role_scores.extend([1 for _ in range(len(self.nested_roles) - len(self.nested_role_scores))])

        entities = [[i, j, entity_itos[k], mention_itos[l], s] for (i, j, k), (_, _, l), s in
                    zip(self.entities, self.mentions, self.entity_scores)]
        ...
        roles = [[i, j, flat_role_itos[k], l] for (i, j, k), l in zip(self.roles, self.role_scores)]
        nested_roles = [[i, j, nested_role_itos[k], l] for (i, j, k), l in
                        zip(self.nested_roles, self.nested_role_scores)]
    ...
    def copy(self):
        ...
        graph = Graph(
            ...
            nested_roles=self.nested_roles.copy(),
        ...
        graph.nested_role_scores = self.nested_role_scores
        return graph
    ...
    def clean(self, relation_directional=False, symmetric_relations=None):
        ...
        nested_roles = [(i, j, k, l) for (i, j, k), l in zip(self.nested_roles, self.nested_role_scores)]
        ...
        self.nested_roles = [(i, j, k) for i, j, k, _ in nested_roles]
        self.nested_role_scores = [l for _, _, _, l in nested_roles]
    ...
    def add_nested_role(self, idx1, idx2, label, score=0, score_norm=0):
        """Add an event-argument link edge to the graph.
        :param idx1 (int): Index of the trigger node.
        :param idx2 (int): Index of the trigger node.
        :param label (int): Index of the role label.
        :param score (float): Label score.
        """
        # assert idx1 < self.trigger_num and idx2 < self.entity_num
        # self.roles.append((idx1, idx2, label))
        if label:
            self.nested_roles.append((idx1, idx2, label))
            self.nested_role_scores.append(score_norm)
        self.nested_role_num = len(self.nested_roles)
        self.graph_local_score += score
    ...
    def to_label_idxs(self, max_entity_num, max_trigger_num,
        ...
        nested_role_idxs = [0] * max_trigger_num * max_trigger_num
        for i, j, nested_role in self.nested_roles:
            nested_role_idxs[i * max_trigger_num + j] = nested_role
        nested_role_mask = [1 if i < self.trigger_num and j < self.trigger_num else 0
                            for i in range(max_trigger_num) for j in range(max_trigger_num)]
