...
instance_fields = [
    ...
    'relation_type_idxs', 'flat_role_type_idxs', 'nested_role_type_idxs',
    'mention_type_idxs',
    'graph', 'entity_num', 'trigger_num', 'event_prompt_idx', 'flat_role_prompt_idx', 'nested_role_prompt_idx'
]
...
batch_fields = [
    ...
    'relation_type_idxs', 'flat_role_type_idxs', 'nested_role_type_idxs',
    'graphs', 'token_nums', 'event_prompt_idx', 'flat_role_prompt_idx', 'nested_role_prompt_idx'
]
...
InstanceLdcEval = namedtuple('InstanceLdcEval', field_names=instance_ldc_eval_fields,
                             defaults=[None] * len(instance_ldc_eval_fields))
Batch = namedtuple('Batch', field_names=batch_fields, defaults=[None] * len(batch_fields))
BatchLdcEval = namedtuple('BatchLdcEval', field_names=batch_ldc_eval_fields,
...
def get_nested_role_types(events):
    labels = [['O'] * len(events) for _ in range(len(events))]
    event_idxs = {event['id']: i for i, event in enumerate(events)}
    for event_idx, event in enumerate(events):
        for arg in event['arguments']:
            if 'Content' in arg['role']:
                event_id2 = arg['entity_id']
                event_idx2 = event_idxs[event_id2]
                labels[event_idx][event_idx2] = arg['role']
    return labels


def get_nested_role_types_new(events):
    labels = [[0] * len(events) for _ in range(len(events))]
    event_idxs = {event['id']: i for i, event in enumerate(events)}
    for event_idx, event in enumerate(events):
        if "Nest" in event['event_type']:
            for arg in event['arguments']:
                if 'Content' in arg['role']:
                    event_id2 = arg['entity_id']
                    event_idx2 = event_idxs[event_id2]
                    labels[event_idx][event_idx2] = 1
        else:
            for i in range(len(events)):  # ensure inner-nest event not pointing to outer-nest event
                labels[event_idx][i] = -100
    return labels


def get_nested_role_list(events, vocab):
    event_idxs = {event['id']: i for i, event in enumerate(events)}
    visited = [[0] * len(events) for _ in range(len(events))]
    role_list = []
    for i, event in enumerate(events):
        for arg in event['arguments']:
            if 'Content' in arg['role']:
                event_idx2 = event_idxs[arg['entity_id']]
                if visited[i][event_idx2] == 0:
                    role_list.append((i, event_idx2, vocab[arg['role']]))
                    visited[i][event_idx2] = 1
    role_list.sort(key=lambda x: (x[0], x[1]))
    return role_list
...
class IEDataset(Dataset):
    def __init__(self, path, max_length=128, gpu=False, ignore_title=False,
                 ...
                 coref=False, symmetric_relations=None, event_identity_only=False, use_prompt=True, nested_mask=False):
        """
        ...
        self.coref = coref
        self.event_identity_only = event_identity_only
        self.use_prompt = use_prompt
        self.nested_mask = nested_mask
        ...
        with open(self.path, 'r', encoding='utf-8') as r:
            for line in r:
                ...
                if self.ignore_title and is_title:
                    title_num += 1
                    continue
                if self.max_length != -1 and inst_len > 128:
                    overlength_num += 1
                    continue
    ...
    def numberize(self, tokenizer, vocabs):
        ...
        flat_role_type_stoi = vocabs['flat_role_type']
        nested_role_type_stoi = vocabs['nested_role_type']
        label_mapping = vocabs["label_mapping"]
        data = []

        prompt_piece = []
        event_prompt_idx = []
        event_type_itos = {i: s for s, i in event_type_stoi.items()}
        if self.use_prompt:
            for event_type_id in range(len(event_type_itos)):
                prompt_piece.append("[EVENT]")
                event_prompt_idx.append(len(prompt_piece))
                event_type_label = label_mapping["trigger"][event_type_itos[event_type_id]]
                prompt_piece = prompt_piece + tokenizer.tokenize(event_type_label)
            prompt_piece.append("[SEP]")

        flat_role_prompt_idx = []
        nested_role_prompt_idx = []
        flat_role_type_itos = {i: s for s, i in flat_role_type_stoi.items()}
        nested_role_type_itos = {i: s for s, i in nested_role_type_stoi.items()}
        if self.use_prompt:
            for role_type_id in range(len(flat_role_type_itos)):
                prompt_piece.append("[ROLE]")
                flat_role_prompt_idx.append(len(prompt_piece))
                role_type_label = label_mapping["role"][flat_role_type_itos[role_type_id]]
                prompt_piece = prompt_piece + tokenizer.tokenize(role_type_label)
            prompt_piece.append("[SEP]")

        if self.use_prompt:
            for role_type_id in range(len(nested_role_type_itos)):
                prompt_piece.append("[NESTEDROLE]")
                nested_role_prompt_idx.append(len(prompt_piece))
                role_type_label = label_mapping["role"][nested_role_type_itos[role_type_id]]
                prompt_piece = prompt_piece + tokenizer.tokenize(role_type_label)
            prompt_piece.append("[SEP]")
        ...
        for inst in self.data:
            ...
            pieces = prompt_piece + pieces

            # Pad word pieces with special tokens
            piece_idxs = tokenizer.encode(pieces,
                                          add_special_tokens=True,
                                          max_length=self.max_length,
                                          truncation=True)  # it will add [CLS] at front and add [SEP] at tail
            ...
            if self.event_identity_only:
                vocabs['trigger_label'] = {'O': 0, 'B-trigger': 1, 'I-trigger': 2}
                trigger_label_idxs = []  # change the label to trigger during identification
                for l in trigger_labels:
                    if l.startswith("B-"):
                        trigger_label_idxs.append(1)
                    elif l.startswith("I-"):
                        trigger_label_idxs.append(2)
                    else:
                        trigger_label_idxs.append(0)
            else:
            ...
            # Argument role
            flat_role_types = get_role_types(entities, events, entity_id_map)
            flat_role_type_idxs = [[flat_role_type_stoi[l] for l in ls]
                                   for ls in flat_role_types]
            flat_role_list = get_role_list(entities, events,
                                           entity_id_map, flat_role_type_stoi)

            # Nested Argument role
            if self.nested_mask:
                nested_role_type_idxs = get_nested_role_types_new(events)
            else:
                nested_role_types = get_nested_role_types(events)
                nested_role_type_idxs = [[nested_role_type_stoi[l] for l in ls]
                                         for ls in nested_role_types]

            if self.relation_mask_self:
                for i in range(len(nested_role_type_idxs)):
                    nested_role_type_idxs[i][i] = -100
            nested_role_list = get_nested_role_list(events, nested_role_type_stoi)
            ...
            graph = Graph(
                ...
                roles=flat_role_list,
                nested_roles=nested_role_list,
            ...
            instance = Instance(
                ...
                flat_role_type_idxs=flat_role_type_idxs,
                nested_role_type_idxs=nested_role_type_idxs,
                graph=graph,
                entity_num=len(entities),
                trigger_num=len(events),
                event_prompt_idx=event_prompt_idx,
                flat_role_prompt_idx=flat_role_prompt_idx,
                nested_role_prompt_idx=nested_role_prompt_idx
            )
    ...
    def collate_fn(self, batch):
        ...
        batch_relation_types, batch_flat_role_types, batch_nested_role_types = [], [], []
        batch_mention_types = []
        batch_graphs = []
        batch_token_lens = []
        ...
        for inst in batch:
            ...
            event_prompt_idx.append((inst.event_prompt_idx))
            flat_role_prompt_idx.append((inst.flat_role_prompt_idx))
            nested_role_prompt_idx.append((inst.nested_role_prompt_idx))
            # for identification
            ...
            for l in inst.flat_role_type_idxs:
                batch_flat_role_types.extend(
                    l + [-100] * (max_entity_num - inst.entity_num))
            batch_flat_role_types.extend(
                [-100] * max_entity_num * (max_trigger_num - inst.trigger_num))

            for l in inst.nested_role_type_idxs:
                batch_nested_role_types.extend(
                    l + [-100] * (max_trigger_num - inst.trigger_num))
            batch_nested_role_types.extend(
                [-100] * max_trigger_num * (max_trigger_num - inst.trigger_num))
        ...
        if self.gpu:
            ...
            batch_flat_role_types = torch.cuda.LongTensor(batch_flat_role_types)
            batch_nested_role_types = torch.cuda.LongTensor(batch_nested_role_types)

            token_nums = torch.cuda.LongTensor(token_nums)
            event_prompt_idx = torch.cuda.LongTensor(event_prompt_idx)
            flat_role_prompt_idx = torch.cuda.LongTensor(flat_role_prompt_idx)
            nested_role_prompt_idx = torch.cuda.LongTensor(nested_role_prompt_idx)
        else:
            ...
            batch_flat_role_types = torch.LongTensor(batch_flat_role_types)
            batch_nested_role_types = torch.LongTensor(batch_nested_role_types)

            token_nums = torch.LongTensor(token_nums)
            event_prompt_idx = torch.LongTensor(event_prompt_idx)
            flat_role_prompt_idx = torch.LongTensor(flat_role_prompt_idx)
            nested_role_prompt_idx = torch.LongTensor(nested_role_prompt_idx)
        ...
        return Batch(
            ...
            flat_role_type_idxs=batch_flat_role_types,
            nested_role_type_idxs=batch_nested_role_types,
            graphs=batch_graphs,
